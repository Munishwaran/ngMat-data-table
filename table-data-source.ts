...
import { TextFilterComponent } from 'src/app/components/dashboard/common/filter/text-filter/text-filter.component';
import { DataTableService } from './../app/components/dashboard/common/data-table/data-table.service';
...
export class MatTableDataSource<T> extends DataSource<(T | Group)> {
  @ViewChild(TextFilterComponent, null) textFilter;

  /** Stream that emits when a new data array is set on the data source. */
  private readonly _data: BehaviorSubject<T[]>;

  /** Stream emitting render data to the table (depends on ordered data changes). */
  private _renderData = new BehaviorSubject<(T | Group)[]>([]);
  // private messageSource = new BehaviorSubject('default message');
  currentSrc = this._renderData.asObservable();
  ...
  caseInsensitiveComparator(valueA, valueB) {
    ...
    var valueALowerCase = valueA ? valueA : "";
    var valueBLowerCase = valueB ? valueB : "";
  ...
  sortData: ((data: T[], sort: MatSort) => T[]) = (data: T[], sort: MatSort): T[] => {
    ...
    return data.sort((a, b) => {
      ...
      let comparatorResult = this.caseInsensitiveComparator(a[active], b[active]);
   ...
   */
  filterPredicate: ((data: T, filter: string) => boolean) = (data: T, filter: any): boolean => {
    // Transform the data into a lowercase string of all property values.
    const accumulator = (currentTerm: string, key: string) => currentTerm + (data as { [key: string]: any })[key];
    const dataStr = Object.keys(data).reduce(accumulator, '').toLowerCase();

    // Transform the filter by converting it to lowercase and removing whitespace.
    const transformedFilter = filter.operand1Input.trim().toLowerCase();

    return dataStr.indexOf(transformedFilter) != -1;
  }

  constructor(initialData: T[] = [], public tableService: DataTableService) {
    super();
  ...
  _updateChangeSubscription() {
    ...
    const groupedData = combineLatest(orderedData, groupChange)
      .pipe(map(([data]) => {
        return this._groupData(data, this.tableId);
      }));
    ...
    this._renderChangesSubscription = paginatedData.subscribe(data => {
      return this._renderData.next(data);
    });
  }
   ...
   */
  _groupData(data: T[], tableId): (T | Group)[] {
    if (!this.groupBy) { return data; }

    const groupedData = this.groupBy.groupData(data.slice(), tableId);
